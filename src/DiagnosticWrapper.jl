# ----------
# Wrapper Module for FourierFlows.Diagnostic Module to Fit the MHDFlows Problem Type
# ----------
...
$(TYPEDFIELDS)
"""
mutable struct Diagnostic{T, N, AbstractProblem} <: AbstractDiagnostic
...
function Diagnostic(calc, prob; freq=1, nsteps=100, ndata=ceil(Int, (nsteps+1)/freq))
  ...
  return Diagnostic{T,ndata,AbstractProblem}(calc, prob, data, t, steps, freq, 1)
end
...
function update!(diag, i)
  i <= length(diag.steps) || extend!(diag)
   ...
   diag.data[i] = diag.calc(diag.prob)
